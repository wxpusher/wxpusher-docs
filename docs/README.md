...
极简推送<br/>（SPT一键推送）|<ul><li>发送消息和接收消息的是同一个人</li><li>非常简单，无需登陆，创建应用等</li><li>简单发送消息能力，无法管理消息和接收者</li><li>适合无经验但是想简单发送消息的用户</li><li>最多一次发送给10个人，具体说明请查看<a href="#/?id=spt">极简推送接口</a></li></ul>



# 方式一：标准推送 :id=standard
## 名词解释
- 应用
...
# 系统对接：跳转到你的系统 :id=setting-url-sign

**系统对接**让用户可以从 WxPusher 一键进入你的系统，跳转时**自动携带用户身份 `uid`**，并可用「验证密钥」做签名防伪造，让用户可以无缝登录，修改设置等作定制化功能。常用于打开控制台、进入会员中心、uid推送下的推送设置等场景。
...
| 配置项 | 说明 |
|------|------|
| 你的系统名称 | 入口显示的文字，如「演示系统」 |
| 你的系统LOGO图标 | 入口显示的图标，正方形、≤500KB、PNG/JPG/WEBP；不配置就不显示图标 |
| 你的系统入口 | 用户点击后跳转的目标地址，**要求公网可以访问**；只有配置了此项，入口才会显示 |
| 验证密钥 | 可选。配置后跳转参数会带 HMAC 签名，供你校验来源真实、防伪造防重放（见「三、验证签名」） |

## 入口显示在哪里

配置好「你的系统入口」后，用户可以从以下位置进入你的系统：

| 入口位置 | 说明 | 示意图 |
|------|------|------|
| 关注 / 订阅引导页 | 分享或引导用户关注的订阅页面。支持添加 `jump=auto` 参数：当满足「用户已登录 + 已订阅该应用 + 配置了系统入口」时，页面会自动携带uid等参数跳转到你的系统，用户无需再点一次，比如：https://wxpusher.zjiecode.com/app/#/follow?type=1&id=141&jump=auto  | ![关注订阅页](../imgs/share-to-follow-app.jpg ':size=170') |
| App 订阅详情页 | WxPusher App「我的订阅 → 订阅详情」页 | ![订阅详情页](../imgs/app-subscribe-detail.jpg ':size=170') |
| App 扩展功能 Tab | WxPusher App 底部「扩展功能」Tab | ![扩展功能](../imgs/app-ext-func.jpg ':size=170') |

> 请注意，只有用户订阅了你的应用，才会显示你的系统入口！！！

## 跳转链接与 uid

用户点击入口时，WxPusher 会跳转到你配置的地址，并在链接上携带该用户的 `uid`，你的系统拿到 `uid` 即可识别用户身份、提供对应服务。

- **未配置验证密钥**：链接只带明文 `uid`，如 `https://your.site/setting?uid=UID_xxx`。
- **配置了验证密钥**：链接会额外带 `ts / nonce / sv / sign` 签名参数（见「四、验证签名」）。

> 强烈建议添加验证密钥，否则请求uid可能被伪造，需要你自行负责。

## 验证签名（防伪造、防重放）

为了让你的系统**验证请求来源真实、并防止链接被伪造和重放**，在「系统对接」里配置一个只有你知道的 `验证密钥`。配置后，WxPusher 会用该密钥对跳转参数做 HMAC-SHA256 签名。

> 若不配置 `验证密钥`，跳转链接只携带明文 `uid`、不带签名（兼容旧行为），强烈建议添加验证密钥。
...
如果你只想尽快收到任务通知，可以参考：[青龙面板使用 WxPusher 极简推送教程](qinglong-wxpusher-spt.md)，扫码获取 SPT 后填入青龙面板即可。
...
- 单条消息的数据长度（字符数）限制：`content` 不超过 **40000**；`summary` 不超过 **100**（各端展示可能截断，不宜过长）；`url` 不超过 **1000**；
- 单条请求最多 **`uids` 2000 个**、**`topicIds` 5 个**；
- 单个 UID 通过 **App 等客户端渠道**接收消息时，系统会按自然日累计用量：**单日约超过 3000 条后不在通知栏提醒**（消息列表仍可显示），**约超过 20000 条后可能不再写入消息列表并停止推送展示**，次日自动恢复；具体阈值以线上策略为准，请合理安排发送频率；
- 发送消息接口默认限流约为 **每 10 秒最多 20 次调用（约合 2 QPS）**，超过会被拦截；部分应用可申请更高配额。若发送量大，建议使用主题群发，或在单次请求中附带多个 `uid`。
